	...
	protected void actionEventPerformedbtnA3(ActionEvent arg0) {
		if(txtCantidad11.getText().matches("[0-9]{1,3}")) {
			...
			if(cantPr11 < c.getCantPr()){
				...
				error("No hay tantos productos en stock", txtCantidad11);
			}
			if(valorpr11 == false) {
				totalpr11 = 0;
			}
			...
			error("Ingresa la cantidad correctamentes", txtCantidad11);
		}
	}
	protected void actionEventPerformedbtnA4(ActionEvent arg0) {
		if(txtCantidad12.getText().matches("[0-9]{1,3}")) {
			c = ac.obtener(6);
			cantPr12 = Integer.parseInt(txtCantidad12.getText().trim());
			if(cantPr12 < c.getCantPr()){
				valorpr12 = true;
				if(valorpr12 == true) {
					totalpr12 = c.getPrecioPr() * cantPr12;
					c.setCantPr(c.getCantPr() - cantPr12);
					ac.actualizarArchivo();
					pr.estado=10;
					estado=1;
				}
				...
				error("No hay tantos productos en stock", txtCantidad12);
			}
			if(valorpr12 == false) {
				totalpr12= 0;
			}
			...
			error("Ingresa la cantidad correctamentes", txtCantidad12);
		}
	}
